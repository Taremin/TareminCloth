...
### 2.3 自己衝突のCoupled協調収束とPost-Relaxation（エッジ過剰伸長抑制の実現）
- **背景と課題**:
  自己衝突（V-T, E-E）も距離拘束や曲げ拘束と同様に反復ループ（Solver Iterations）内で協調して解かれない場合、反復ループ外で一方的に衝突反発変位が確定し、エッジが引き伸ばされたままサブステップが終了してしまいます。特に強い圧縮下では、実測で最大 9.5% 以上の過剰なエッジ伸長（布の不自然な伸び）が発生していました。
...
- **ソフトピン（0.0 < weight < 1.0 / ウェイトペイントによる緩やかな固定）の設計妥当性**:
  ...
  - したがって、「反復ループ内でピン目標へ引き寄せた後、サブステップ終端の衝突処理で安全圏へ退避させて位置確定する」というパイプライン順序は、**ソフトピンの貫通・めり込み回避を最優先とした合理的かつ自然な設計**となっています。
...
### 3.5 自己衝突における作用・反作用の完全対称分配（固定小数点アトミック加算による運動量保存）
- **物理的背景**:
...
- **検証結果**:
  二重布対称性テスト（`test_self_collision_symmetry.py`）により、等質量布および異質量布（2:1）において、相対誤差 0.7% 台の厳密な運動量保存（完全対称反発）を実証済み。GPU並列競合によるメッシュ崩壊を起こすことなく、非対称めり込みが完全に解消されています。
...
- **現在の設計**: 固定小数点（$10^6$ 倍スケール）の整数アトミック加算（`atomicAdd`）を用いた専用アキュムレータバッファと Solve → Apply 分離パイプラインを導入し、GPUデータ競合を起こすことなく完全対称分配と運動量保存を達成（詳細は [3.5節](#35-自己衝突における作用反作用の完全対称分配固定小数点アトミック加算による運動量保存) を参照）。
...
└─────────────────────────────────┘       └─────────────────────────────────┘
┌─────────────────────────────────┐       ┌─────────────────────────────────┐
│ 完全な運動量保存 (作用反作用)   │       │ 完全対称アトミック分配          │
│ ・V-T/E-Eで相手頂点にも         │ ----> │ ・固定小数点 atomicAdd 蓄積     │
│   重心座標・質量比で等量分配    │       │ ・運動量保存達成 (第3.5節)      │
└─────────────────────────────────┘       └─────────────────────────────────┘
┌─────────────────────────────────┐       ┌─────────────────────────────────┐
...
### 5.1 完全非貫通保証（バリア関数法）との乖離と幾何ペナルティ・CCD法の妥協
- **本来の理想 (IPC / Barrier Methods)**:
  Incremental Potential Contact (IPC: [Li et al. 2020](https://ipc-sim.github.io/)) 等に代表される最新の接触理論では、接触距離がゼロになる前に滑らかに無限大へ発散するバリアポテンシャル関数（Cubic Barrier 等）を導入し、適応型直線探索（Line Search CCD）とニュートン法によって**「幾何学的に交差・貫通が絶対に発生しない状態」を厳密に保証**します。
...
- **トレードオフと今後の課題**:
  超高速衝突や極度の挟み込み時に一時的なめり込みが発生するリスクは残りますが、サブステップ数を増やす（10〜20）ことで実用上ほぼ完全に貫通を回避できるバランスを確保しています。

### 5.2 今後のアルゴリズム改善ロードマップ（将来の研究課題）
1. **Persistent Contacts キャッシュによるNarrowphase軽量化**:
   超高解像度メッシュにおいて、サブステップ開始時に接近ペアのみをGPU動的バッファに抽出し、反復ループ内での空間ハッシュ探索コストを完全排除する次世代最適化の検討。
2. **適応型SDF解像度**:
