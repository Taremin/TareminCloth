...
2. **2段階軽量自己衝突ループ（Persistent E-E Contacts）**:
   サブステップ開始時に空間ハッシュ走査で接近エッジペアを抽出・キャッシュし、反復ループ内ではそのエッジペアに対する軽量なE-E接触ペナルティを距離拘束と同調解決（Coupled XPBD）。メッシュ解像度に依存しない稜線バリアを維持しつつ、サブステップ終端でトポロジーCCD（V-T, E-E）を実行して貫通を完全遮断するパイプラインへの発展。
3. **適応型SDF解像度**:
