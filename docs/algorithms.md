...
└─────────────────────────────────┘       └─────────────────────────────────┘
┌─────────────────────────────────┐       ┌─────────────────────────────────┐
│ 完全な運動量保存 (作用反作用)   │       │ 完全対称アトミック分配 [解消済] │
│ ・V-T/E-Eで相手頂点にも         │ ----> │ ・固定小数点 atomicAdd 蓄積     │
│   重心座標・質量比で等量分配    │       │ ・GPU競合ゼロで運動量保存達成   │
└─────────────────────────────────┘       └─────────────────────────────────┘
┌─────────────────────────────────┐       ┌─────────────────────────────────┐
...
### 5.1 自己衝突の協調収束とPost-Relaxation（エッジ過剰伸長抑制の実現 [解消済]）
- **本来の理想**:
  自己衝突（V-T, E-E）も距離拘束や曲げ拘束と全く同様にXPBD拘束方程式 $C_{collision}(\mathbf{x}) \ge 0$ として定式化され、反復ループ（Solver Iterations）の内部で距離拘束と協調して解かれるべきである。反復ループ外で一方的に衝突反発変位が確定すると、エッジが引き伸ばされたままサブステップが終了し、極度の圧縮下で布に不自然な伸び（実測で最大 9.5% 以上のエッジ過剰伸長）が発生してしまう。
- **実装された設計 (Taremin Cloth)**:
  実機ベンチマーク（`benchmarks/benchmark_coupled_self_collision.py`）により、性能・品質の両面から以下の2段階の選択式協調モード（`coupled_self_collision_mode`）を実装・配備：
  1. **緩和モード (`RELAXATION` / 推奨標準デフォルト)**:
     - 自己衝突処理（Solve + Apply）の直後に、**距離拘束を2反復（2 iters）再適用（Post-Relaxation）**。
     - 自己衝突によって外側へ押し出された頂点のエッジ長が即座に整流化され、衝突探索の負荷軽減効果と相殺することで、**通常FPSの低下ほぼゼロ（153.2 FPS維持）のまま、最大エッジ伸長率を約5割抑制（9.55% → 4.13%、-56.7%低減）**。
  2. **完全協調モード (`FULL_COUPLED` / 最高品質)**:
     - XPBD反復ループの各イテレーション内で自己衝突を同調解決し、ループ終了後に仕上げとして距離拘束を1反復適用（ハイブリッド方式）。
     - 衝突反発とエッジ拘束が反復ループ内で互いに押し合って調和し、**100FPS超のリアルタイム性（107.1 FPS）を維持しつつ、最大エッジ伸長率を約7割抑制（9.55% → 2.94%、-69.2%低減という全方式中最高精度）**。
- **検証結果**:
  単体・結合テスト（`test_coupled_self_collision.py`）およびベンチマークスクリプトにおいて、OFF時の過剰なエッジ引き伸ばしが大幅に抑制されることを確認済み。

### 5.2 作用・反作用の完全対称分配（固定小数点アトミック加算による運動量保存の実現 [解消済]）
- **本来の理想**:
...
- **実装された設計 (Taremin Cloth)**:
  ...
  - **E-E（辺対辺）**: 2線分最短パラメータ $(s, t)$ に基づき、自エッジ2頂点に $(1-s, s)$、相手エッジ2頂点に $-(1-t, t)$ を質量比重みでアトミック加算。重複評価は動的エッジ間で $index < j$ により排除しつつ、ピン相手との接触も完全保証。
...
- **検証結果**:
  二重布対称性テスト（`test_self_collision_symmetry.py`）により、等質量布および異質量布（2:1）において、相対誤差 0.7% 台の厳密な運動量保存（完全対称反発）を実証済み。GPU並列競合によるメッシュ崩壊を起こすことなく、非対称めり込みが完全に解消された。

### 5.3 ピン拘束と自己衝突・エッジ衝突の優先度設計（完全ピンとソフトピンの挙動）
- **完全固定ピン（weight = 1.0 / Grab操作・完全固定ボーン）の挙動**:
  ...
  - そのため、反復ループ後の `Final Pin Pass`（[`dispatch.rs`](../crates/cloth_core/src/simulation/dispatch.rs)）実行後も、完全固定ピンの位置は衝突力によって動かされることなく、目標位置が絶対的に維持されます。
- **ソフトピン（0.0 < weight < 1.0 / ウェイトペイントによる緩やかな固定）の設計妥当性**:
  - 物理的には無限質量の壁ではなく、目標位置へバネで引かれるコンプライアンスを持った拘束（Attachment Constraint）です。
  - 強い衝突反発力を受けた際にピン目標位置から押し戻される挙動は、力学的に極めて自然です。
  - むしろ、サブステップ終端の自己衝突・エッジ衝突の「後」にピン位置スナップを強制適用してしまうと、衝突処理でせっかくめり込みを解消して押し戻された頂点が再び目標位置（布やコライダーの内部）へ引き戻され、貫通・交差が再発してしまいます。
  - したがって、「反復ループ内でピン目標へ引き寄せた後、サブステップ終端の衝突処理で安全圏へ退避させて位置確定する」という現在のパイプライン順序は、**ソフトピンの貫通・めり込み回避を最優先とした合理的かつ自然な設計**となっています。

### 5.4 今後のアルゴリズム改善ロードマップ（乖離解消への道筋）
1. **GPUアトミック加算による作用・反作用の対称分配 [完了]**:
   `atomicAdd` を用いた固定小数点（$10^6$ スケール）変位アキュムレータバッファと Solve → Apply 分離パイプラインを導入し、V-T/E-E 接触における作用・反作用の完全対称分配と運動量保存を達成（二重布テスト相対誤差 0.7%）。
2. **Coupled自己衝突協調モード (Relaxation / Full Coupled) [完了]**:
   自己衝突直後の距離拘束再適用（Post-Relaxation）および反復内同調ハイブリッドを実装。FPS低下ゼロで伸びを約5割抑制する標準モードと、100FPS超を保ちつつ伸びを約7割抑制する最高品質モードを選択可能化。
3. **Persistent Contacts キャッシュによるNarrowphase軽量化**:
   さらなる超高解像度メッシュにおいて、サブステップ開始時に接近ペアのみをGPU動的バッファに抽出し、反復ループ内での空間ハッシュ探索コストを完全排除する次世代最適化の検討。
4. **適応型SDF解像度**:
