...
- **質量ブレンドの連続仕様 (`set_pin_target`, `simulation/mod.rs`)**:
  - ピンウェイト `w` に対し `inv_mass = original_inv_masses * (1 - w)` の連続ブレンドを適用します（`w=0` → 不変、`w=1` → 完全固定）。旧仕様の `weight > 0.5` 二値判定は、連続スライダーや減衰に対する不連続応答（0.49/0.51 での重力・衝突挙動の反転）を招くため廃止されました。
  - 質量側の連続化により、旧仕様の `mix` スナップ（反復内外で毎サブステップ約30回適用され、任意の `w > 0` が目標へ吸着していた）から、布側の抵抗が定常的に残る力比べへ移行しました。
- **コンプライアントピン拘束 (`pin.wgsl`, XPBD形式)**:
  - ピンは `C(x) = |x - target|` のXPBD拘束として距離拘束と同一形式で解かれます（`inv_mass` とコンプライアンスの両方が `w` で連続変化）。旧仕様の `mix` スナップは、反復内外で毎サブステップ約30回適用されるため任意の `w > 0` が目標へ吸着し、布側の抵抗が定常的に残りませんでした。
  - 基準コンプライアンス `PIN_COMPLIANCE_REF = 1e-6`（構造剛性1000相当）により、`w=0.5` で布の構造拘束と同等に競合します。`w=1`（または元質量0）は従来通りのスナップ経路を維持し、Grab/Pピンの挙動は不変です。
...
### 5.3 平滑化ブラシの張力アシスト（幾何学的弛み逃がし）の妥協
- **本来の理想 (塑性・摩擦ヒステリシス)**:
  実際のアイロンがけは熱と圧力で繊維を塑性変形させ、摩擦ヒステリシスで形状を保持します。理想的には曲げレスト角・自然長の塑性更新（ヒートセット）で平坦形状を定着させるべきです。
- **現在の実装 (Taremin Cloth の妥協と選定理由)**:
  本モデルは純粋弾性（XPBD拘束+弾性平衡）のため、局所平滑化だけでは余剰長が残り、リリース後に元の座屈へ弾性復帰します。そこで平滑化ターゲットに放射方向の弱い幾何学的拡張（`TENSION_EXPAND = 0.05`、falloff・strength スケール、1move=1反復）を合成し、弛みをパッチ外へ逃がしています。物理保存則を破る外力ではなく、ソフトピンによる力比べの範囲内の目標位置操作に留めています。
- **トレードオフと今後の課題**:
  全体圧縮が大きい皺は完全消失しません（広半径での分散撫でと併用）。塑性レスト更新は将来の研究課題とし、本妥協の parity は `tests/core/test_smooth_parity.py` で保証します。
---
